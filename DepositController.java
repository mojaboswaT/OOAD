...
public class DepositController {
    ...
    public void initialize() {
        accountBox.setCellFactory(cb -> new ListCell<>() {
            @Override protected void updateItem(Account item, boolean empty) {
                ...
                else setText(item.getAccountNumber() + " - " + item.getType() + " (BWP " + item.getBalance() + ")");
            }
        });
        accountBox.setButtonCell(new ListCell<>() {
            @Override protected void updateItem(Account item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) setText(null);
                else setText(item.getAccountNumber() + " - " + item.getType());
            }
        });
        refreshAccounts();
    }
    ...
    public void handleDeposit() {
        try {
            ...
            service.deposit(acc.getId(), amt, referenceField.getText());
            message.setText("Deposited BWP " + amt + " to " + acc.getAccountNumber());
            refreshAccounts();
        } catch (Exception e) {
